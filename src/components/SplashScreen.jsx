  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:9999,background:'#000',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',opacity:exit?0:1,transition:'opacity .5s ease',overflow:'hidden'}}>
      <style>{`
        ...
        @keyframes sp-lineout{to{opacity:0}}
      ...
      {/* Heartbeat trace */}
      <svg viewBox="0 0 400 100" preserveAspectRatio="none" style={{position:'absolute',top:'50%',left:0,width:'100%',height:80,transform:'translateY(-50%)',animation:'sp-lineout .4s ease 0.85s forwards',zIndex:2,pointerEvents:'none'}}>
        <defs>
          <linearGradient id="pulseG" x1="0" y1="0" x2="400" y2="0" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor="#4B8CF5"/>
            <stop offset="50%" stopColor="#fff"/>
            <stop offset="100%" stopColor="#17E5B0"/>
          </linearGradient>
        </defs>
        <path
          d="M -20 50 H 150 L 165 15 L 180 85 L 195 30 L 210 50 H 420"
          fill="none" stroke="url(#pulseG)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
          pathLength="1" style={{strokeDasharray:1,strokeDashoffset:1,animation:'sp-draw .9s cubic-bezier(.4,0,.2,1) forwards'}}
        />
      </svg>

      {/* Logo */}
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',position:'relative',opacity:0,animation:'sp-logoin .6s cubic-bezier(.22,1,.36,1) 0.85s forwards'}}>
        <div style={{position:'relative',display:'flex',alignItems:'center',lineHeight:1,overflow:'hidden'}}>
          ...
          <span style={{display:'inline-flex',alignItems:'center',margin:'0 2px',position:'relative',top:1,opacity:0,animation:'sp-bolt .4s cubic-bezier(.34,1.56,.64,1) 1.05s forwards'}}>
          ...
          <div style={{position:'absolute',top:0,bottom:0,width:'40%',background:'linear-gradient(90deg,transparent,rgba(255,255,255,0.08),transparent)',animation:'sp-shine .8s 1.25s ease forwards',pointerEvents:'none'}}/>
        </div>
        <div style={{marginTop:16,fontSize:10,fontWeight:600,letterSpacing:'0.22em',textTransform:'uppercase',color:'rgba(235,240,255,0.3)',opacity:0,animation:'sp-sub .4s ease 1.35s forwards'}}>
