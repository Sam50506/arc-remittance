...
const metadata = { name: 'Arc Remittance', description: 'Send USDC globally', url: 'https://arc-remittance.vercel.app', icons: ['https://arc-remittance.vercel.app/logo192.png'] };
const wagmiConfig = defaultWagmiConfig({ chains: [arcChain], projectId: WALLETCONNECT_PROJECT_ID, metadata });
const modal = createWeb3Modal({ wagmiConfig, projectId: WALLETCONNECT_PROJECT_ID, chains: [arcChain] });
...
function App() {
  ...
  useEffect(() => {
    modal.subscribeProvider(({ provider, providerType, address, isConnected, chainId }) => {
      if (isConnected && provider && address) {
        setWallet(address);
        setProviderName("WalletConnect");
        setWalletProvider(provider);
        loadPayments(address, provider);
      }
    });
  }, []);
  ...
  async function connectWithExtension() {
    ...
    if (!provider) { setStatus("No wallet extension found!"); return; }
