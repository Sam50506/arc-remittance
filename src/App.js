...
function AppInner() {
  ...
  const handleMulti=async()=>{setShowConfirm(false);const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{const recipients=valid.map(r=>ethers.getAddress(r.addr));const amounts=valid.map(r=>ethers.parseUnits(parseFloat(r.amount).toString(),18));const countries=valid.map(r=>r.country||'');const total=amounts.reduce((a,b)=>a+b,0n);setStatus({type:'info',msg:'Sending to '+valid.length+' recipients...'});const{remit}=getC();const tx=await remit.batchSend(recipients,amounts,countries,{value:total,gasLimit:250000});const txHash=tx.hash;await Promise.race([tx.wait(),new Promise((_,r)=>setTimeout(()=>r(new Error('timeout')),30000))]).catch(()=>null);valid.forEach((r)=>{const rec={hash:txHash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'confirmed'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});awardCashback(txHash);});setStatus({type:'success',msg:'Sent to '+valid.length+' recipients in one transaction!'});setMulti([{addr:'',amount:'',country:''}]);setTimeout(refreshBal,3000);setTimeout(refreshBal,6000);}catch(e){console.log('ERROR:',e);setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};const handleCreateInv=async()=>{if(!address){setStatus({type:'error',msg:'Connect your wallet first'});return;}if(!invPayer||!invAmt){setStatus({type:'error',msg:'Fill required fields'});return;}setLoading(true);setStatus({type:'info',msg:'Creating invoice...'});try{const id='0x'+Array.from(crypto.getRandomValues(new Uint8Array(32))).map(b=>b.toString(16).padStart(2,'0')).join('');const invoice={id,creator:address,payer:invPayer.trim(),amount:invAmt,description:invDesc,country:invCtry,paid:false};await sbInsert('invoices',invoice);const saved=ls('arc_invoices',[]);lsSave('arc_invoices',[{...invoice,desc:invDesc,ts:Math.floor(Date.now()/1000)},...saved.slice(0,99)]);setInvId(id);setStatus({type:'success',msg:'Invoice created. Share the ID with your client.'});setInvPayer('');setInvAmt('');setInvDesc('');}catch(e){setStatus({type:'error',msg:'Failed: '+e.message});}finally{setLoading(false);}};
