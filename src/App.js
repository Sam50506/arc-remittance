...
function AppInner() {
  ...
  const handleMulti=async()=>{setShowConfirm(false);const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{const isMetaMask=walletName==='MetaMask'||walletName==='Brave'||window.ethereum?.isMetaMask;const feeData=await provider.getFeeData().catch(()=>null);const gasPrice=feeData?.gasPrice||ethers.parseUnits('21','gwei');if(isMetaMask){let nonce=await provider.getTransactionCount(address,'pending');setStatus({type:'info',msg:'Confirm '+valid.length+' transactions in MetaMask...'});const txPromises=valid.map((r,idx)=>signer.sendTransaction({to:ethers.getAddress(r.addr.toLowerCase()),value:ethers.parseUnits(parseFloat(r.amount).toString(),18),gasLimit:21000,gasPrice,nonce:nonce+idx}));const txs=await Promise.all(txPromises);txs.forEach((tx,idx)=>{const r=valid[idx];const rec={hash:tx.hash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'pending'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});awaitReceipt(provider,tx.hash).then(receipt=>{const confirmed=receipt==null?'pending':receipt.status===1?'confirmed':'failed';if(confirmed==='confirmed')awardCashback(tx.hash);setTxns(prev=>{const u=prev.map(t=>t.hash===tx.hash?{...t,status:confirmed}:t);lsSave('arc_txhistory_'+address,u);return u;});});});}else{const{remit}=getC();const recipients=valid.map(r=>ethers.getAddress(r.addr.toLowerCase()));const amounts=valid.map(r=>ethers.parseUnits(parseFloat(r.amount).toString(),18));const countries=valid.map(r=>r.country||'');const total=amounts.reduce((a,b)=>a+b,0n);const tx=await remit.batchSend(recipients,amounts,countries,{value:total,gasLimit:250000,gasPrice});await tx.wait();valid.forEach((r)=>{const rec={hash:tx.hash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'confirmed'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});awardCashback(tx.hash);});}setStatus({type:'success',msg:'Sent to '+valid.length+' recipients!'});setMulti([{addr:'',amount:'',country:''}]);setTimeout(refreshBal,3000);setTimeout(refreshBal,6000);}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};
