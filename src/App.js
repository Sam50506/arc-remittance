...
/* ─── Global CSS (from App_UI.js, extended with features-specific classes) ─── */
const CSS = `
...
const IC = {
  ...
  MultiSend:() => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3l4 4-4 4"/><path d="M3 7h18"/><path d="M7 21l-4-4 4-4"/><path d="M21 17H3"/></svg>,
  Contacts: () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
  ...
  Rates:    () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>,
...
async function awaitReceipt(provider, hash, ms = 45000) {
  ...
  while (Date.now() < end) {
    try { const r = await provider.getTransactionReceipt(hash); if (r) return r; } catch (_) {}
...
function WalletPicker({ onPick, onClose }) {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--b1)', borderRadius: 16, padding: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--tx1)', fontFamily: 'var(--fd)' }}>Choose your wallet</div>
      {options.map((o, i) => (
        <button key={i} onClick={() => onPick(o.type, o.p)}
          style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--elev)', border: '1px solid var(--b1)', borderRadius: 12, padding: '13px 16px', cursor: 'pointer', fontSize: 14, fontWeight: 600, color: 'var(--tx1)', width: '100%', textAlign: 'left' }}>
          {o.label}
        </button>
      ))}
      <button onClick={onClose} style={{ fontSize: 13, color: 'var(--tx3)', background: 'none', border: 'none', cursor: 'pointer', marginTop: 4 }}>← Back</button>
    </div>
  );
}
...
export default function App() {
  ...
  const [cCtry, setCCtry] = useState('Pakistan');
  ...
  useEffect(() => {
    ...
    if (invParam) {
      try {
        ...
        if (!saved.find(i => i.id === inv.id)) lsSave('arc_invoices', [inv, ...saved]);
        setPayId(inv.id);
        setTab('pay');
  ...
  const loadContractHistory = useCallback(async () => {
    try {
      const remit = new ethers.Contract(REMIT_ADDR, REMIT_ABI, signer);
  ...
  const handleSend = async () => {
    ...
    try {
      const value    = ethers.parseUnits(sendAmt, 18);
      ...
      awaitReceipt(provider, tx.hash).then(receipt => {
        ...
        setTxns(prev => { const u = prev.map(t => t.hash === tx.hash ? { ...t, status: confirmed } : t); lsSave('arc_txhistory', u); return u; });
      });
  ...
  const handleCreateInv = async () => {
    ...
    try {
      const id = '0x' + Array.from(crypto.getRandomValues(new Uint8Array(32))).map(b => b.toString(16).padStart(2,'0')).join('');
      const invoice = { id, creator: address, payer: invPayer.trim(), amount: invAmt, description: invDesc, country: invCtry, paid: false };
  ...
  const statusClass = () => {
    ...
    return m[status.type] || m.info;
  };

  const statusIcon = () => {
    if (!status) return null;
    if (status.type === 'success') return <IC.Check />;
    if (status.type === 'error')   return <IC.X />;
    return <IC.Alert />;
  };

  const nav = (id) => { setTab(id); setMobOpen(false); };

  /* ── Tab definitions ── */
  const TABS = [
    { id: 'send',     label: 'Send',      Icon: IC.Send      },
    { id: 'multi',    label: 'Multi-Send', Icon: IC.MultiSend },
    { id: 'invoice',  label: 'Invoice',   Icon: IC.Invoice   },
    { id: 'pay',      label: 'Pay',       Icon: IC.Pay       },
    { id: 'contacts', label: 'Contacts',  Icon: IC.Contacts  },
    { id: 'schedule', label: 'Scheduled', Icon: IC.Schedule  },
    { id: 'history',  label: 'History',   Icon: IC.History   },
    { id: 'rates',    label: 'Rates',     Icon: IC.Rates     },
    { id: 'fees',     label: 'Compare',   Icon: IC.Compare   },
    { id: 'settings', label: 'Settings',  Icon: IC.Settings  },
  ];

  const PAGE_META = {
    send:     { title: 'Send USDC',           sub: 'Transfer USDC instantly. Arc USDC is native — sends like ETH.' },
    multi:    { title: 'Multi-Send',           sub: 'Send USDC to multiple recipients in one session.' },
    invoice:  { title: 'Create Invoice',       sub: 'Request USDC payment. Stored in Supabase, payable from any device.' },
    pay:      { title: 'Pay Invoice',          sub: 'Enter an invoice ID to look it up and pay instantly.' },
    contacts: { title: 'Contacts',             sub: 'Save wallet addresses for quick access when sending.' },
    schedule: { title: 'Scheduled Payments',   sub: 'Set up recurring transfers. Click Execute to pre-fill Send.' },
    history:  { title: 'Transaction History',  sub: 'All outgoing transfers from your connected wallet.' },
    rates:    { title: 'Exchange Rates',        sub: '1 USDC = 1 USD · Rates via exchangerate-api.com' },
    fees:     { title: 'Fee Comparison',        sub: 'Sending $100 internationally. See what you actually pay.' },
    settings: { title: 'Settings',              sub: 'Manage preferences, network info, and local data.' },
  };

  /* ── Toggle style ── */
  const togStyle = { width: 40, height: 22, borderRadius: 999, background: dm ? '#4B8CF5' : '#192030', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0, border: 'none' };
  const togKnob  = { position: 'absolute', top: 3, left: dm ? 21 : 3, width: 16, height: 16, borderRadius: '50%', background: '#fff', transition: 'left 0.2s' };
        ...
        <div className="ap-connect">
          <div className="ap-connect-card">
            ...
            <div className="ap-connect-btns">
              {showPicker ? (
                <WalletPicker onPick={(type, p) => { setShowPicker(false); connectBrowser(type, p); }} onClose={() => setShowPicker(false)} />
              ) : (
                <>
                  <button className="ap-btn ap-btn-primary" style={{ marginTop: 0 }} onClick={() => setShowPicker(true)}>
                    <IC.Wallet /> Connect Wallet
                  </button>
                  <div className="ap-connect-divider">or</div>
                  <button className="ap-btn ap-btn-full-outline" onClick={connectWC}>
            ...
            {status && (
              <div className={statusClass()} style={{ marginTop: 16, marginBottom: 0, textAlign: 'left' }}>
                {statusIcon()}<span>{status.msg}</span>
              </div>
            )}
        ...
        <div className="ap-app">
          ...
          <aside className={`ap-sidebar${mobOpen ? ' mob-open' : ''}`}>
            ...
            <nav className="ap-nav">
              ...
              {TABS.map(({ id, label, Icon }) => (
                <div key={id} className={`ap-nav-item${tab === id ? ' active' : ''}`} onClick={() => nav(id)}>
                  <Icon />{label}
                </div>
              ))}
            </nav>
            <div className="ap-sidebar-foot">
              ...
              <div className="ap-wallet-pill">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 10, color: 'var(--tx3)', fontWeight: 600, letterSpacing: '.06em' }}>
                    {walletName.replace(/[^\w\s]/g, '').trim().toUpperCase() || 'WALLET'}
                  </div>
          ...
          <main className="ap-content">
            ...
            <header className="ap-topbar">
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ fontFamily: 'var(--fd)', fontWeight: 700, fontSize: 17, letterSpacing: '-.2px' }}>{PAGE_META[tab]?.title}</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button className="ap-btn ap-btn-icon" onClick={doDisconnect} title="Disconnect"><IC.X /></button>
              </div>
            </header>
            ...
            <div className="ap-page">
              <div className="ap-page-enter">
                <div className="ap-page-sub">{PAGE_META[tab]?.sub}</div>

                {/* Status banner */}
                {status && (
                  <div className={statusClass()} style={{ marginBottom: 20 }}>
                    {statusIcon()}<span>{status.msg}</span>
                  </div>
                )}
                  ...
                  <div className="ap-card">
                    {contacts.length > 0 && (
                      <div className="ap-field">
                        <label className="ap-label">Quick Select</label>
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          {contacts.map(c => (
                            <button key={c.id} className="ap-btn ap-btn-sec" style={{ fontSize: 12, padding: '6px 12px' }}
                              onClick={() => { setSendTo(c.address); setSendCtry(c.country); }}>
                    ...
                    <div className="ap-field">
                      <label className="ap-label">Recipient Address</label>
                      <input className="ap-input" placeholder="0x..." value={sendTo} onChange={e => setSendTo(e.target.value)} />
                    </div>
                    <div className="ap-field">
                      <label className="ap-label">Amount (USDC)</label>
                      <input className="ap-input" type="number" placeholder="0.00" value={sendAmt} onChange={e => setSendAmt(e.target.value)} />
                    </div>
                    <div className="ap-field" style={{ marginBottom: 6 }}>
                      <label className="ap-label">Destination Country</label>
                      <select className="ap-select" value={sendCtry} onChange={e => setSendCtry(e.target.value)}>
                        <option value="">Select country</option>
                        {COUNTRIES.map(c => <option key={c} value={c}>{FLAG[c]} {c}</option>)}
                      </select>
                    </div>
                    {converted() && (
                      <div style={{ background: 'rgba(75,140,245,.08)', border: '1px solid rgba(75,140,245,.2)', borderRadius: 12, padding: '14px 16px', marginBottom: 14 }}>
                        <div style={{ fontSize: 12, color: 'var(--bl2)', fontWeight: 700, marginBottom: 4 }}>RECIPIENT GETS APPROXIMATELY</div>
                        <div style={{ fontSize: 22, fontWeight: 800, fontFamily: 'var(--fd)', color: 'var(--tx1)' }}>{FLAG[sendCtry]} {converted()} {CURRENCY[sendCtry]}</div>
                        <div style={{ fontSize: 12, color: 'var(--tx2)', marginTop: 4 }}>1 USDC = {rates[CURRENCY[sendCtry]]?.toFixed(2)} {CURRENCY[sendCtry]} · Live rate</div>
                      </div>
                    )}
                    {sendAmt && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', fontSize: 13, color: 'var(--tx3)', borderTop: '1px solid var(--b0)', marginBottom: 6 }}>
                        <span>Estimated gas</span><span style={{ color: 'var(--cy)', fontWeight: 600 }}>&lt; $0.01</span>
                      </div>
                    )}
                    <button className="ap-btn ap-btn-primary" onClick={handleSend} disabled={loading}>
                      {loading ? 'Sending…' : `Send ${sendAmt || '0'} USDC`}
                    </button>
                  </div>
                )}
                  ...
                  <div className="ap-card">
                    {multi.map((r, i) => (
                      <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 10, alignItems: 'flex-start' }}>
                        <div style={{ flex: 2 }}>
                          {i === 0 && <label className="ap-label">Address</label>}
                          <input className="ap-input" style={{ marginBottom: 0 }} placeholder="0x..." value={r.addr}
                            onChange={e => { const v = e.target.value; setMulti(p => p.map((x, j) => j === i ? { ...x, addr: v } : x)); }} />
                        </div>
                        <div style={{ flex: 1 }}>
                          {i === 0 && <label className="ap-label">USDC</label>}
                          <input className="ap-input" style={{ marginBottom: 0 }} type="number" placeholder="0.00" value={r.amount}
                            onChange={e => { const v = e.target.value; setMulti(p => p.map((x, j) => j === i ? { ...x, amount: v } : x)); }} />
                        </div>
                        <div style={{ flex: 1 }}>
                          {i === 0 && <label className="ap-label">Country</label>}
                          <select className="ap-select" style={{ marginBottom: 0 }} value={r.country}
                            onChange={e => { const v = e.target.value; setMulti(p => p.map((x, j) => j === i ? { ...x, country: v } : x)); }}>
                            {COUNTRIES.map(c => <option key={c} value={c}>{c}</option>)}
                          </select>
                        </div>
                        {multi.length > 1 && (
                          <button className="ap-btn ap-btn-danger" style={{ marginTop: i === 0 ? 22 : 0 }}
                            onClick={() => setMulti(p => p.filter((_, j) => j !== i))}>X</button>
                        )}
                      </div>
                    ))}
                    <button className="ap-btn ap-btn-ghost" style={{ width: '100%', padding: 10, borderRadius: 9, marginTop: 4, border: '1px dashed var(--b1)', justifyContent: 'center' }}
                      onClick={() => setMulti(p => [...p, { addr: '', amount: '', country: 'Pakistan' }])}>
                      + Add Recipient
                    </button>
                    <div style={{ margin: '14px 0', padding: '12px 14px', background: 'var(--elev)', borderRadius: 10, border: '1px solid var(--b1)', fontSize: 14, color: 'var(--tx1)' }}>
                      Total: <strong>${multi.reduce((s, r) => s + (parseFloat(r.amount) || 0), 0).toFixed(2)} USDC</strong>
                      {' '}to <strong>{multi.filter(r => r.addr && r.amount).length}</strong> recipients
                    </div>
                    <button className="ap-btn ap-btn-primary" onClick={handleMulti} disabled={loading}>
                  ...
                  <div>
                    <div className="ap-card">
                      <div className="ap-field">
                        ...
                        <input className="ap-input" placeholder="0x..." value={invPayer} onChange={e => setInvPayer(e.target.value)} />
                      </div>
                      <div className="ap-field">
                        <label className="ap-label">Amount (USDC)</label>
                        <input className="ap-input" type="number" placeholder="500" value={invAmt} onChange={e => setInvAmt(e.target.value)} />
                      </div>
                      <div className="ap-field">
                        <label className="ap-label">Description</label>
                        <input className="ap-input" placeholder="Logo design - May 2026" value={invDesc} onChange={e => setInvDesc(e.target.value)} />
                      </div>
                      <div className="ap-field" style={{ marginBottom: 6 }}>
                        <label className="ap-label">Your Country</label>
                        <select className="ap-select" value={invCtry} onChange={e => setInvCtry(e.target.value)}>
                          {COUNTRIES.map(c => <option key={c} value={c}>{FLAG[c]} {c}</option>)}
                        </select>
                      </div>
                      <button className="ap-btn ap-btn-primary" onClick={handleCreateInv} disabled={loading}>
                        {loading ? 'Creating…' : 'Create Invoice'}
                      </button>
                    </div>
                    {invId && (
                      <div className="ap-card">
                        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--cy)', marginBottom: 6 }}>Invoice created successfully</div>
                        <div className="ap-code-label">Invoice ID</div>
                        <div style={{ fontSize: 13, color: 'var(--tx2)', marginBottom: 8 }}>Share this ID with your client so they can pay from any device.</div>
                        <div className="ap-code-box">{invId}</div>
                        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                          <button className="ap-btn ap-btn-sec" onClick={() => navigator.clipboard?.writeText(invId)}>
                            <IC.Copy /> Copy ID
                          </button>
                          <button className="ap-btn ap-btn-primary" style={{ width: 'auto', padding: '9px 16px', marginTop: 0 }}
                            onClick={() => { setPayId(invId); setTab('pay'); }}>
                            Pay this Invoice
                          </button>
                        </div>
                      </div>
                    ...
                    {ls('arc_invoices', []).length > 0 && (
                      <div className="ap-card">
                        <div className="ap-code-label" style={{ marginBottom: 12 }}>Recent Invoices</div>
                        {ls('arc_invoices', []).slice(0, 5).map((inv, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--b0)' }}>
                            ...
                            <button className="ap-btn ap-btn-sec" style={{ fontSize: 12, padding: '6px 10px' }}
                              onClick={() => { setPayId(inv.id); setTab('pay'); }}>Pay</button>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div>
                    <div className="ap-card">
                      <div className="ap-field" style={{ marginBottom: 6 }}>
                        ...
                        <input className="ap-input" placeholder="0x..." value={payId} onChange={e => setPayId(e.target.value)} />
                      </div>
                      <button className="ap-btn ap-btn-primary" onClick={handlePayInv} disabled={loading || !payId}>
                        {loading ? 'Processing…' : 'Look Up & Pay'}
                      </button>
                    </div>
                    {payDet && (
                      <div className="ap-card">
                        ...
                        <div className="ap-invoice-detail">
                          {[
                            ['Amount',      `${fmtUsdc(payDet.amount)} USDC`],
                            ['Description', payDet.description],
                            ['Country',     `${FLAG[payDet.country] || ''} ${payDet.country}`],
                            ['From',        short(payDet.creator)],
                          ].map(([k, v]) => (
                ...
                {tab === 'contacts' && (
                  <div>
                    <div className="ap-card">
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <div>
                          ...
                          <input className="ap-input" style={{ marginBottom: 0 }} placeholder="Ahmed" value={cName} onChange={e => setCName(e.target.value)} />
                        </div>
                        <div>
                          <label className="ap-label">Country</label>
                          <select className="ap-select" style={{ marginBottom: 0 }} value={cCtry} onChange={e => setCCtry(e.target.value)}>
                            {COUNTRIES.map(c => <option key={c} value={c}>{FLAG[c]} {c}</option>)}
                          </select>
                        </div>
                      </div>
                      <div className="ap-field" style={{ marginTop: 14 }}>
                        <label className="ap-label">Wallet Address</label>
                        <input className="ap-input" placeholder="0x..." value={cAddr} onChange={e => setCAddr(e.target.value)} />
                      </div>
                    ...
                    {contacts.length > 0 && (
                      <div className="ap-card">
                        <div className="ap-code-label" style={{ marginBottom: 12 }}>Saved Contacts ({contacts.length})</div>
                        {contacts.map(c => (
                          <div key={c.id} className="ap-hist-item">
                            <div style={{ width: 40, height: 40, borderRadius: 12, background: 'var(--elev)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 800, color: 'var(--bl2)', flexShrink: 0 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                              <button className="ap-btn ap-btn-sec" style={{ fontSize: 12 }}
                                onClick={() => { setSendTo(c.address); setSendCtry(c.country); setTab('send'); }}>Send</button>
                              <button className="ap-btn ap-btn-danger"
                                onClick={() => setContacts(p => p.filter(x => x.id !== c.id))}>Remove</button>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                {tab === 'schedule' && (
                  <div>
                    <div className="ap-card">
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <div>
                          <label className="ap-label">Recipient</label>
                          <input className="ap-input" style={{ marginBottom: 0 }} placeholder="0x..." value={newSched.addr}
                            onChange={e => setNewSched(s => ({ ...s, addr: e.target.value }))} />
                        </div>
                        <div>
                          <label className="ap-label">Amount (USDC)</label>
                          <input className="ap-input" style={{ marginBottom: 0 }} type="number" placeholder="0.00" value={newSched.amount}
                            onChange={e => setNewSched(s => ({ ...s, amount: e.target.value }))} />
                        </div>
                        <div>
                          <label className="ap-label">Country</label>
                          <select className="ap-select" style={{ marginBottom: 0 }} value={newSched.country}
                            onChange={e => setNewSched(s => ({ ...s, country: e.target.value }))}>
                            {COUNTRIES.map(c => <option key={c} value={c}>{FLAG[c]} {c}</option>)}
                          </select>
                        </div>
                        <div>
                          <label className="ap-label">Frequency</label>
                          <select className="ap-select" style={{ marginBottom: 0 }} value={newSched.freq}
                            onChange={e => setNewSched(s => ({ ...s, freq: e.target.value }))}>
                      ...
                      <div className="ap-field" style={{ marginTop: 14 }}>
                        <label className="ap-label">Next Date</label>
                        <input className="ap-input" type="date" value={newSched.next}
                          onChange={e => setNewSched(s => ({ ...s, next: e.target.value }))} />
                      </div>
                      <button className="ap-btn ap-btn-primary" onClick={() => {
                    ...
                    {scheds.length > 0 && (
                      <div className="ap-card">
                        <div className="ap-code-label" style={{ marginBottom: 12 }}>Active Schedules</div>
                        {scheds.map(s => (
                          <div key={s.id} className="ap-hist-item">
                            <div style={{ flex: 1 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                              <button className="ap-btn ap-btn-sec" style={{ fontSize: 12 }}
                                onClick={() => { setSendTo(s.addr); setSendAmt(s.amount); setSendCtry(s.country); setTab('send'); setStatus({ type: 'info', msg: 'Pre-filled. Review and confirm.' }); }}>
                                Execute
                              </button>
                              <button className="ap-btn ap-btn-danger"
                                onClick={() => setScheds(p => p.filter(x => x.id !== s.id))}>Remove</button>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                {tab === 'history' && (
                  <div>
                    {allTxns.length > 0 && (
                      <div className="ap-card">
                        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--tx1)', fontFamily: 'var(--fd)', marginBottom: 4 }}>Transfer Volume</div>
                        <div style={{ fontSize: 11, color: 'var(--tx3)', marginBottom: 16 }}>Total sent: ${totalSent.toFixed(2)} USDC</div>
                    ...
                    <div className="ap-card">
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                        <div style={{ fontSize: 15, fontWeight: 700, fontFamily: 'var(--fd)', color: 'var(--tx1)' }}>
                          Transactions ({allTxns.length})
                        </div>
                        <button className="ap-btn ap-btn-sec" onClick={loadContractHistory}>Refresh</button>
                      </div>
                      {allTxns.length === 0
                        ? <div className="ap-hist-empty">No transactions yet</div>
                        : allTxns.map((t, i) => (
                          <div key={i} className="ap-hist-item">
                            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(75,140,245,.11)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: '#4B8CF5', flexShrink: 0, fontWeight: 700 }}>↑</div>
                            <div style={{ flex: 1, marginLeft: 14 }}>
                            ...
                            <div style={{ textAlign: 'right', marginRight: 8 }}>
                              <div style={{ fontWeight: 800, color: 'var(--tx1)', fontSize: 15 }}>
                            ...
                            {t.hash && (
                              <a href={`https://testnet.arcscan.app/tx/${t.hash}`} target="_blank" rel="noreferrer"
                                style={{ fontSize: 13, color: 'var(--bl)', textDecoration: 'none', padding: '4px 8px' }}>View</a>
                            )}
                          </div>
                        ))
                      }
                      ...
                      {COUNTRIES.map(c => {
                        ...
                        return (
                          <div key={c} style={{ background: 'var(--elev)', border: '1px solid var(--b1)', borderRadius: 12, padding: '13px 15px' }}>
                            ...
                            <div style={{ fontSize: 17, fontWeight: 800, fontFamily: 'var(--fd)', color: 'var(--tx1)', marginTop: 2 }}>
                              {rate ? rate.toLocaleString('en', { maximumFractionDigits: 1 }) : '…'}
                            </div>
                      ...
                      <table className="ap-table">
                        ...
                        <tbody>
                          {[
                            ...
                            { name: 'SWIFT / Bank', fee: '$25 to $45', speed: '3–5 days', save: '...' },
                            ...
                            <tr key={i} className={r.best ? 'best-row' : ''}>
                              <td>{r.name}</td>
                              <td>{r.fee}</td>
                              <td>{r.speed}</td>
                              <td style={{ fontWeight: 700, color: r.save !== '...' ? 'var(--cy)' : 'var(--tx3)' }}>{r.save}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    </div>
                    <div className="ap-compare-footer">ArcPay saves you up to $44.99 per $100 transaction</div>
                    <div style={{ marginTop: 14, background: 'var(--dcy)', border: '1px solid rgba(23,229,176,.15)', borderRadius: 12, padding: 18, textAlign: 'center' }}>
                      <div style={{ fontSize: 12, color: 'var(--cy)', fontWeight: 700, marginBottom: 4 }}>ANNUAL SAVINGS vs BANK WIRE at $500/month</div>
                      <div style={{ fontSize: 34, fontWeight: 800, fontFamily: 'var(--fd)', color: 'var(--tx1)', letterSpacing: '-1px' }}>$2,699</div>
                    </div>
                  </div>
                )}
                    ...
                    {[
                      {
                        ...
                        el: <button style={togStyle} onClick={() => setDm(d => !d)}><div style={togKnob} /></button>
                      },
                      {
                        label: 'Default Send Country', sub: 'Pre-selected when you open Send',
                        el: (
                          <select className="ap-select" style={{ marginBottom: 0, width: 'auto', minWidth: 160 }}
                            value={defCtry} onChange={e => { setDefCtry(e.target.value); setSendCtry(e.target.value); }}>
                    ...
                    <div style={{ padding: '18px 0', borderBottom: '1px solid var(--b0)' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '6px 16px', fontSize: 13 }}>
                        {[
                          ...
                          ['USDC',      `${USDC_ADDR.slice(0,10)}…`],
                          ['Contract',  `${REMIT_ADDR.slice(0,10)}…`],
                        ].map(([k, v]) => (
                          <React.Fragment key={k}>
                            <span style={{ color: 'var(--tx2)' }}>{k}</span>
                            <span style={{ fontFamily: 'monospace', color: 'var(--tx1)' }}>{v}</span>
                          </React.Fragment>
                        ))}
                      </div>
                    </div>
                    <div style={{ padding: '18px 0' }}>
                      ...
                      <button className="ap-btn ap-btn-danger" onClick={() => {
                        if (window.confirm('Clear all local data?')) {
                          setContacts([]); setScheds([]); setTxns([]);
                          setStatus({ type: 'success', msg: 'Data cleared' });
                        }
                ...
                <div style={{ textAlign: 'center', marginTop: 24, fontSize: 12, color: 'var(--tx3)' }}>
                  ArcPay · Arc Testnet · Chain ID {ARC_CHAIN_ID} ·{' '}
                  <a href="https://testnet.arcscan.app" target="_blank" rel="noreferrer" style={{ color: 'var(--bl)' }}>Explorer</a>
                </div>
              </div>
            </div>
          </main>
          ...
          <nav className="ap-botnav">
            {TABS.slice(0, 5).map(({ id, label, Icon }) => (
              <div key={id} className={`ap-bot-item${tab === id ? ' active' : ''}`} onClick={() => nav(id)}>
                <Icon />{label}
              </div>
            ))}
          </nav>
