...
function AppInner() {
  ...
  const loadContractHistory=useCallback(async()=>{if(!address)return;try{const r=await fetch('https://testnet.arcscan.app/api?module=account&action=txlist&address='+address+'&sort=desc');const d=await r.json();if(d.message!=='OK'||!d.result)return;const allExplorer=d.result.filter(t=>t.isError==='0').map(t=>{const isReceived=t.to.toLowerCase()===address.toLowerCase()&&t.from.toLowerCase()!==address.toLowerCase();return{hash:t.hash,recipient:t.to,sender:t.from,amount:parseFloat(ethers.formatUnits(t.value,18)).toFixed(2),country:'',timestamp:parseInt(t.timeStamp),status:'confirmed',received:isReceived};});setContractTxns(allExplorer);}catch(e){console.log('explorer fetch failed:',e);}},[address]);// eslint-disable-line
