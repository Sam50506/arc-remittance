...
function AppInner() {
  ...
  const handleMulti=async()=>{setShowConfirm(false);const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{console.log('Step 1: getting contract');const{remit}=getC();console.log('Step 2: remit',remit);console.log('Step 3: batchSend type',typeof remit.batchSend);const recipients=valid.map(r=>{try{return ethers.getAddress(r.addr);}catch{return ethers.getAddress(r.addr.toLowerCase());}});console.log('Step 4: recipients',recipients);const amounts=valid.map(r=>ethers.parseUnits(parseFloat(r.amount).toString(),18));const countries=valid.map(r=>r.country||'');const total=amounts.reduce((a,b)=>a+b,0n);console.log('Step 5: total',total.toString());setStatus({type:'info',msg:'Sending to '+valid.length+' recipients...'});const baseFee=ethers.parseUnits('21','gwei');console.log('Step 6: calling batchSend');const tx=await remit.batchSend(recipients,amounts,countries,{value:total,gasLimit:250000,maxFeePerGas:baseFee,maxPriorityFeePerGas:baseFee});console.log('Step 7: tx',tx.hash);await tx.wait();valid.forEach((r)=>{const rec={hash:tx.hash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'confirmed'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});awardCashback(tx.hash);});setStatus({type:'success',msg:'Sent to '+valid.length+' recipients in one transaction!'});setMulti([{addr:'',amount:'',country:''}]);setTimeout(refreshBal,3000);setTimeout(refreshBal,6000);}catch(e){console.log('ERROR:',e);setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};
