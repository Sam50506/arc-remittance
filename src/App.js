...
function AppInner() {
  ...
  const[tab,setTab]=useState('send');const[status,setStatus]=useState(null);const[loading,setLoading]=useState(false);const[mobOpen,setMobOpen]=useState(false);const[dm,setDm]=useState(()=>{const saved=ls('arc_dm');return saved===true||saved==='true';});
