...
function AppInner() {
  ...
  useEffect(()=>{if(tab==='history'&&signer)loadContractHistory();},[tab,signer,loadContractHistory]);useEffect(()=>{if(tab==='rewards'&&address)fetchMyClaims();},[tab,address]);useEffect(()=>{if(tab==='settings'&&address&&address.toLowerCase()==='0x9e086e6c07d5108ce40d84e9df1ce43caedd2306'){sbSelect('cashback_claims','status=eq.pending&select=id').then(rows=>setPendingClaimsCount(rows?.length||0)).catch(()=>{});}},[tab,address]);useEffect(()=>{if(tab==='settings'&&address&&address.toLowerCase()==='0x9e086e6c07d5108ce40d84e9df1ce43caedd2306'){sbSelect('cashback_claims','status=eq.pending&select=id').then(rows=>setPendingClaimsCount(rows?.length||0)).catch(()=>{});}},[tab,address]);
  ...
  const renderHistory=()=>(<div>{allTxns.length>0&&(<div className="ap-card"><div className="ap-card-title">Transfer Volume</div><div style={{marginTop:16}}><ResponsiveContainer width="100%" height={160}><AreaChart data={chartData} margin={{top:8,right:8,left:0,bottom:0}}><defs><linearGradient id="cg" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3B82C4" stopOpacity={0.2}/><stop offset="95%" stopColor="#3B82C4" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="var(--b0)"/><XAxis dataKey="label" tick={{fontSize:11,fill:'var(--tx3)'}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:10,fill:'var(--tx3)'}} axisLine={false} tickLine={false} tickFormatter={v=>v>=1000?Math.round(v/1000)+'k':v} width={35}/><Tooltip contentStyle={{background:'var(--card)',border:'1px solid var(--b1)',borderRadius:10,fontSize:13,color:'var(--tx1)'}}/><Area type="monotone" dataKey="sent" stroke="#3B82C4" fill="url(#cg)" strokeWidth={2} name="Sent (USDC)"/></AreaChart></ResponsiveContainer></div></div>)}<div className="ap-card"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}><div><div className="ap-card-title">Transactions ({allTxns.length})</div><div style={{fontSize:12,color:'var(--tx3)',marginTop:2}}>Total sent: {totalSent.toFixed(2)} USDC</div></div><div style={{display:'flex',gap:8}}><button className="ap-btn ap-btn-sec" onClick={exportCSV} style={{fontSize:12,padding:'7px 12px'}}>Export CSV</button><button className="ap-btn ap-btn-sec" onClick={async()=>{await refreshPendingTxns();loadContractHistory();}} style={{fontSize:12,padding:'7px 12px'}}>Refresh</button><button className="ap-btn ap-btn-sec" onClick={()=>setManageTxns(m=>!m)} style={{fontSize:12,padding:'7px 12px',color:manageTxns?'var(--re)':undefined}}>{manageTxns?'Done':'Manage'}</button></div></div>{manageTxns&&<div style={{fontSize:12,color:'var(--tx2)',background:'var(--elev)',borderRadius:10,padding:'8px 12px',marginBottom:12}}>Deleting transactions only removes them from this device. Your cashback rewards are not affected.</div>}{allTxns.length===0?<div style={{textAlign:'center',color:'var(--tx3)',padding:'32px 0',fontSize:14}}>No transactions yet</div>:allTxns.map((t,i)=>(<div key={i} className="ap-hist-row"><div className="ap-hist-icon"><IC.Send/></div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:'var(--tx1)',fontSize:14,display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>{t.country&&<span className="ap-cc">{CC[t.country]||'?'}</span>}{t.country||'Transfer'}<span className={txBadge(t.status)}>{t.status||'pending'}</span></div><div style={{fontSize:11,color:'var(--tx3)',fontFamily:'monospace',marginTop:3,overflow:'hidden',textOverflow:'ellipsis'}}>{short(t.recipient)}</div></div><div style={{textAlign:'right',flexShrink:0}}><div style={{fontWeight:700,color:'var(--tx1)',fontSize:14}}>{parseFloat(typeof t.amount==='bigint'||(typeof t.amount==='object'&&t.amount!==null)?ethers.formatUnits(BigInt(t.amount.toString()),18):t.amount).toFixed(2)} USDC</div><div style={{fontSize:11,color:'var(--tx3)',marginTop:3}}>{fmtDate(t.timestamp)}</div></div>{t.hash&&!t.hash.startsWith('0xdemo')&&<a href={'https://testnet.arcscan.app/tx/'+t.hash} target="_blank" rel="noreferrer" style={{fontSize:12,color:'var(--ac)',textDecoration:'none',padding:'4px 6px',flexShrink:0}}><IC.Ext/></a>}{manageTxns&&<button onClick={()=>{if(window.confirm('Delete this transaction from your history?')){const updated=txns.filter((_,j)=>j!==i);lsSave('arc_txhistory_'+address,updated);setTxns(updated);}}} style={{background:'var(--re)',border:'none',color:'#fff',cursor:'pointer',padding:'4px 8px',flexShrink:0,fontSize:12,borderRadius:8,fontWeight:600}}>Delete</button>}</div>))}</div></div>);
