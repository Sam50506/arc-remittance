...
function AppInner() {
  ...
  const handleMulti=async()=>{setShowConfirm(false);const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{let gasPrice;try{const feeData=await provider.getFeeData();gasPrice=feeData?.gasPrice||ethers.parseUnits('1','gwei');}catch{gasPrice=ethers.parseUnits('1','gwei');}for(const r of valid){setStatus({type:'info',msg:'Sending to '+short(r.addr)+'...'});const value=ethers.parseUnits(parseFloat(r.amount).toString(),18);const nonce=await provider.getTransactionCount(address,'pending');const tx=await signer.sendTransaction({to:r.addr,value,gasPrice,nonce});const rec={hash:tx.hash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'pending'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});awaitReceipt(provider,tx.hash).then(receipt=>{const confirmed=receipt==null?'pending':receipt.status===1?'confirmed':'failed';if(confirmed==='confirmed')awardCashback(tx.hash);setTxns(prev=>{const u=prev.map(t=>t.hash===tx.hash?{...t,status:confirmed}:t);lsSave('arc_txhistory_'+address,u);return u;});});}setStatus({type:'success',msg:'Sent to '+valid.length+' recipients'});setMulti([{addr:'',amount:'',country:''}]);setTimeout(refreshBal,3000);setTimeout(refreshBal,6000);setTimeout(refreshBal,10000);}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};
