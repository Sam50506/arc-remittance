...
function AppInner() {
  ...
  const handleMulti=async()=>{setShowConfirm(false);if(typeof window.misesEthereum!=='undefined'&&window.ethereum?.isMetaMask){const origReq=window.ethereum.request.bind(window.ethereum);window.ethereum.request=async(args)=>{if(args.method==='eth_getBalance'||args.method==='eth_call'){try{const r=await fetch('https://rpc.testnet.arc.network',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',method:args.method,params:args.params,id:1})});const d=await r.json();return d.result;}catch{}}return origReq(args);};}const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{const{remit}=getC();const recipients=valid.map(r=>ethers.getAddress(r.addr));const amounts=valid.map(r=>ethers.parseUnits(parseFloat(r.amount).toString(),18));const countries=valid.map(r=>r.country||'');const total=amounts.reduce((a,b)=>a+b,0n);setStatus({type:'info',msg:'Sending to '+valid.length+' recipients...'});const iface=new ethers.Interface(['function batchSend(address[] recipients, uint256[] amounts, string[] countries) payable']);const data=iface.encodeFunctionData('batchSend',[recipients,amounts,countries]);const bp=new ethers.BrowserProvider(window.ethereum);const signerAddr=await(await bp.getSigner()).getAddress();const txParams={from:signerAddr,to:'0xEC605Cea7C1270C01A3e7B869f762CfDAB8c8E41',data,value:'0x'+total.toString(16),gas:'0x3D090',gasPrice:'0x3B9ACA00'};const txHash=await window.ethereum.request({method:'eth_sendTransaction',params:[txParams]});console.log('tx hash:',txHash);if(!txHash)throw new Error('Transaction rejected by wallet - no hash returned');const receipt=await Promise.race([bp.waitForTransaction(txHash,1,30000),new Promise((_,reject)=>setTimeout(()=>reject(new Error('timeout')),30000))]).catch(async()=>{console.log('timed out, assuming success');return{transactionHash:txHash,status:1};});console.log('receipt:',receipt);valid.forEach((r)=>{const rec={hash:txHash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'confirmed'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});awardCashback(txHash);});setStatus({type:'success',msg:'Sent to '+valid.length+' recipients in one transaction!'});setMulti([{addr:'',amount:'',country:''}]);setTimeout(refreshBal,3000);setTimeout(refreshBal,6000);}catch(e){console.log('ERROR:',e);setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};
