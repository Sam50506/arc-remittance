...
function CountrySelect({value,onChange}){
  ...
  if(isOKX){
    return(<>
      ...
      {open&&<div style={{position:'fixed',inset:0,zIndex:999,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'flex-end'}} onClick={()=>setOpen(false)}>
        <div style={{background:'var(--card)',width:'100%',maxHeight:'70vh',overflowY:'auto',borderRadius:'20px 20px 0 0',padding:'16px 0',marginTop:'30vh'}} onClick={e=>e.stopPropagation()}>
  ...
  return(
    <div style={{position:'relative',display:'inline-block',flexShrink:0}}>
      ...
      <select value={value} onChange={e=>onChange(e.target.value)} style={{position:'relative',zIndex:1,opacity:0.01,cursor:'pointer',width:140,height:40,fontSize:16,border:'none',background:'transparent'}}>
...
function OKXSelect({value, onChange, options, style}){
  ...
  if(isOKX){
    return(<>
      <div onClick={()=>setOpen(o=>!o)} style={{...style,background:'var(--elev)',border:'1px solid var(--b1)',borderRadius:12,padding:'12px 14px',fontSize:14,color:'var(--tx1)',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
  ...
  return(<select className="ap-select" value={value} onChange={e=>onChange(e.target.value)} style={style}>{options.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select>);
}
