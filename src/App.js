...
function AppInner() {
  ...
  const connectBrowser=async(type,provObj)=>{try{const isEip6963=type&&type.startsWith('eip6963_');console.log('connectBrowser type:',type,'provObj:',provObj,'isOkx:',provObj?.isOkxWallet,'window.okx:',!!window.okxwallet);const eth=isEip6963&&provObj?provObj:window.okxwallet&&(provObj?.isOkxWallet||provObj?.isOKExWallet)?window.okxwallet:provObj||await getProvider();console.log('eth selected:',eth?.isMetaMask,eth?.isOkxWallet);if(!eth){setStatus({type:'error',msg:'No wallet found. Install MetaMask.'});return;}await eth.request({method:'eth_requestAccounts'});const bp=new ethers.BrowserProvider(eth,{name:'Arc Testnet',chainId:ARC_CHAIN_ID});await ensureArc(eth);let name='Browser Wallet';if(eth.isMises||(window.mises?.ethereum===eth))name='Mises';else if(eth.isMetaMask&&!eth.isBraveWallet)name='MetaMask';else if(eth.isBraveWallet)name='Brave';else if(eth.isCoinbaseWallet)name='Coinbase';else if(eth.isOkxWallet||eth.isOKExWallet)name='OKX';setWalletName(name);await finaliseConnect(bp);}catch(e){setStatus({type:'error',msg:e.message||'Connection failed'});}};
