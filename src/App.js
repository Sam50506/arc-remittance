...
const ArcPayLogo = ({ size = 36 }) => (
  ...
  {icon:'⭐',title:'Rewards',desc:'Earn cashback on every confirmed transaction. Claim when you reach 1 USDC.'},
...
const ALL_COUNTRIES=['Argentina','Australia','Bangladesh','Brazil','Cambodia','Canada','Chile','China','Colombia','Egypt','Ethiopia','Ghana','India','Indonesia','Japan','Kenya','Malaysia','Mexico','Morocco','Nepal','Nigeria','Pakistan','Peru','Philippines','Poland','Romania','Russia','Saudi Arabia','Singapore','South Africa','South Korea','Sri Lanka','Tanzania','Thailand','Turkey','Uganda','Ukraine','United Arab Emirates','United Kingdom','Vietnam'].sort();
...
function AppInner() {
  ...
  const[contacts,setContacts]=useState([]);const[cName,setCName]=useState('');const[cAddr,setCAddr]=useState('');const[cCtry,setCCtry]=useState('');
  ...
  const loadContractHistory=useCallback(async()=>{if(!address)return;try{const r=await fetch('https://testnet.arcscan.app/api?module=account&action=txlist&address='+address+'&sort=desc');const d=await r.json();if(d.message!=='OK'||!d.result)return;const allExplorer=d.result.filter(t=>t.isError==='0'&&parseInt(t.value)>0).map(t=>{const isReceived=t.to.toLowerCase()===address.toLowerCase()&&t.from.toLowerCase()!==address.toLowerCase();return{hash:t.hash,recipient:t.to,sender:t.from,amount:parseFloat(ethers.formatUnits(t.value,18)).toFixed(2),country:'',timestamp:parseInt(t.timeStamp),status:'confirmed',received:isReceived};});setContractTxns(allExplorer);}catch(e){console.log('explorer fetch failed:',e);}},[address]);// eslint-disable-line
      ...
      for(const tx of pending){
        try{
          ...
          if(r&&r.blockNumber){
            ...
            if(idx>=0){updated[idx]={...updated[idx],status:confirmed};if(confirmed==='confirmed')awardCashback(tx.hash);}
          }
        }catch(_){}
      }
  ...
  const allTxns=[...txns,...contractTxns.filter(r=>!txns.find(l=>l.hash===r.hash))];
  ...
  const renderContacts=()=>(<div><div className="ap-card"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><div><div className="ap-card-title">Contacts ({contacts.length})</div><div style={{fontSize:12,color:'var(--tx3)',marginTop:2}}>Your saved wallet addresses</div></div><div style={{display:'flex',gap:8}}>{contacts.length>0&&<button className="ap-btn ap-btn-sec" style={{fontSize:12,padding:'7px 12px',color:manageContacts?'var(--re)':undefined}} onClick={()=>{setManageContacts(m=>!m);setSelectedContacts([]);}}>{manageContacts?'Done':'Manage'}</button>}<button className="ap-btn ap-btn-primary" style={{fontSize:13,padding:'8px 16px',whiteSpace:'nowrap',width:'auto'}} onClick={()=>setShowAdd(s=>!s)}>{showAdd?'Cancel':'+ Add'}</button></div></div>{showAdd&&<div style={{background:'var(--elev)',borderRadius:12,padding:14,marginBottom:12}}><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}><div><div className="ap-label">Name</div><input className="ap-input" style={{marginBottom:0}} placeholder="Sam" value={cName} onChange={e=>setCName(e.target.value)}/></div><div><div className="ap-label">Country</div><select className="ap-select" style={{marginBottom:0}} value={cCtry} onChange={e=>setCCtry(e.target.value)}><option value="">Select Country</option>{COUNTRIES.map(co=><option key={co} value={co}>{co}</option>)}</select></div></div><div className="ap-label">Wallet Address</div><input className="ap-input" placeholder="0x..." value={cAddr} onChange={e=>setCAddr(e.target.value)} style={{marginBottom:10}}/><button className="ap-btn ap-btn-primary" style={{width:'100%'}} onClick={()=>{if(!cName.trim()||cAddr.trim().length!==42){setStatus({type:'error',msg:'Enter a valid name and address'});return;}setContacts(p=>[{id:Date.now(),name:cName.trim(),address:cAddr.trim(),country:cCtry},...p]);setCName('');setCAddr('');setCCtry('');setShowAdd(false);setStatus({type:'success',msg:'Contact saved'});}}>Save Contact</button></div>}{contacts.length>0&&<input value={cSearch} onChange={e=>setCSearch(e.target.value)} placeholder="Search contacts..." style={{width:'100%',padding:'9px 12px',borderRadius:10,border:'1px solid var(--b1)',background:'var(--elev)',color:'var(--tx1)',fontSize:13,outline:'none',marginBottom:12,boxSizing:'border-box'}}/>}{manageContacts&&<div style={{marginBottom:12}}><div style={{fontSize:12,color:'var(--tx2)',background:'var(--elev)',borderRadius:10,padding:'8px 12px',marginBottom:8}}>Select contacts to delete.</div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button onClick={()=>setSelectedContacts(contacts.map(c=>c.id))} className="ap-btn ap-btn-sec" style={{fontSize:12,padding:'5px 10px'}}>Select All</button><button onClick={()=>setSelectedContacts([])} className="ap-btn ap-btn-sec" style={{fontSize:12,padding:'5px 10px'}}>Deselect All</button>{selectedContacts.length>0&&<button onClick={()=>{if(window.confirm('Delete '+selectedContacts.length+' contacts?')){setContacts(p=>p.filter(c=>!selectedContacts.includes(c.id)));setSelectedContacts([]);setManageContacts(false);}}} style={{background:'var(--re)',border:'none',color:'#fff',cursor:'pointer',padding:'5px 12px',fontSize:12,borderRadius:8,fontWeight:600}}>Delete {selectedContacts.length} Selected</button>}</div></div>}{contacts.filter(ct=>!cSearch||ct.name.toLowerCase().includes(cSearch.toLowerCase())||ct.address.toLowerCase().includes(cSearch.toLowerCase())).map(ct=>(<div key={ct.id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 0',borderBottom:'1px solid var(--b0)'}}>{manageContacts?<input type="checkbox" checked={selectedContacts.includes(ct.id)} onChange={e=>setSelectedContacts(p=>e.target.checked?[...p,ct.id]:p.filter(x=>x!==ct.id))} style={{width:18,height:18,flexShrink:0}}/>:<div style={{width:44,height:44,borderRadius:14,background:addrColor(ct.address),display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:800,color:'#fff',flexShrink:0}}>{ct.name[0].toUpperCase()}</div>}<div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:'var(--tx1)',fontSize:14,display:'flex',alignItems:'center',gap:6}}>{ct.country&&<span className="ap-cc">{CC[ct.country]||'?'}</span>}{ct.name}</div><div style={{fontSize:11,color:'var(--tx3)',fontFamily:'monospace',marginTop:2,overflow:'hidden',textOverflow:'ellipsis'}}>{ct.address}</div></div><div style={{display:'flex',flexDirection:'column',gap:6,flexShrink:0}}><button className="ap-btn ap-btn-primary" style={{fontSize:12,padding:'6px 14px'}} onClick={()=>{setSendTo(ct.address);setSendCtry(ct.country);setTab('send');}}>Send</button><button onClick={()=>{setCAddr(ct.address);setCName(ct.name);setCCtry(ct.country||'');setShowAdd(true);}} style={{background:'none',border:'1px solid var(--b1)',borderRadius:8,padding:'6px 8px',cursor:'pointer',color:'var(--tx2)',fontSize:12,textAlign:'center'}}>Edit Contact</button></div></div>))}</div>{[...new Set(txns.slice(0,20).map(t=>t.recipient).filter(Boolean))].slice(0,5).length>0&&<div className="ap-card"><div className="ap-card-title">Recent Recipients</div><div style={{fontSize:12,color:'var(--tx3)',marginBottom:12}}>Quickly save from your recent transactions</div>{[...new Set(txns.slice(0,20).map(t=>t.recipient).filter(Boolean))].slice(0,5).map((addr,i)=>{const saved=contacts.find(ct=>ct.address.toLowerCase()===addr.toLowerCase());return(<div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:'1px solid var(--b0)'}}><div style={{width:44,height:44,borderRadius:14,background:addrColor(addr),display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,color:'#fff',fontWeight:700,fontSize:14}}>{saved?saved.name[0].toUpperCase():addr.slice(2,4).toUpperCase()}</div><div style={{flex:1,minWidth:0,overflow:'hidden'}}>{saved?<div style={{fontWeight:700,color:'var(--tx1)',fontSize:14}}>{saved.name}</div>:<div/>}<div style={{fontSize:11,color:'var(--tx3)',fontFamily:'monospace',overflow:'hidden',textOverflow:'ellipsis'}}>{addr.slice(0,10)}...{addr.slice(-6)}</div></div><div style={{display:'flex',flexDirection:'column',gap:6,flexShrink:0}}><button className="ap-btn ap-btn-primary" style={{fontSize:12,padding:'6px 14px'}} onClick={()=>{setSendTo(addr);setTab('send');}}>Send</button>{saved?<button style={{background:'none',border:'1px solid var(--b1)',borderRadius:8,padding:'6px 8px',cursor:'pointer',color:'var(--tx2)',fontSize:12,textAlign:'center'}} onClick={()=>{setCAddr(saved.address);setCName(saved.name);setCCtry(saved.country||'');setShowAdd(true);}}>Edit Contact</button>:<button style={{background:'none',border:'1px solid var(--b1)',borderRadius:8,padding:'6px 8px',cursor:'pointer',color:'var(--tx2)',fontSize:12,textAlign:'center'}} onClick={()=>{setCAddr(addr);setShowAdd(true);}}>Save Contact</button>}</div></div>);})}</div>}</div>);
const renderSchedule=()=>(<div><div className="ap-card"><div className="ap-card-title">Schedule Payment</div><div className="ap-card-sub">Set up recurring payment reminders.</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}><div><div className="ap-label">Recipient</div><input className="ap-input" style={{marginBottom:0}} placeholder="0x..." value={newSched.addr} onChange={e=>setNewSched(s=>({...s,addr:e.target.value}))}/></div><div><div className="ap-label">Amount (USDC)</div><input className="ap-input" style={{marginBottom:0}} type="number" placeholder="0.00" value={newSched.amount} onChange={e=>setNewSched(s=>({...s,amount:e.target.value}))}/></div><div><div className="ap-label">Country</div><CountrySelect value={newSched.country} onChange={v=>setNewSched(s=>({...s,country:v}))}/></div><div><div className="ap-label">Frequency</div><select className="ap-select" style={{marginBottom:0}} value={newSched.freq} onChange={e=>setNewSched(s=>({...s,freq:e.target.value}))}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}><div><div className="ap-label">Next Date</div><input className="ap-input" style={{marginBottom:0}} type="date" value={newSched.next} onChange={e=>setNewSched(s=>({...s,next:e.target.value}))}/></div><div><div className="ap-label">Time</div><input className="ap-input" style={{marginBottom:0}} type="time" value={newSched.time} onChange={e=>setNewSched(s=>({...s,time:e.target.value}))}/></div></div><button className="ap-btn ap-btn-primary" onClick={()=>{if(!newSched.addr||!newSched.amount||!newSched.next){setStatus({type:'error',msg:'Fill all required fields'});return;}setScheds(p=>[{id:Date.now(),...newSched},...p]);setNewSched({addr:'',amount:'',country:'',freq:'weekly',next:''});setStatus({type:'success',msg:'Payment scheduled'});}}>Schedule Payment</button></div>{scheds.length>0&&(<div className="ap-card"><div className="ap-card-title">Active Schedules</div><div className="ap-div"/>{scheds.map(s=>(<div key={s.id} style={{display:'flex',alignItems:'center',gap:12,padding:'13px 0',borderBottom:'1px solid var(--b0)'}}><div style={{flex:1}}><div style={{fontWeight:700,color:'var(--tx1)',fontSize:14}}>{s.country&&<span className="ap-cc" style={{marginRight:6}}>{CC[s.country]||'?'}</span>}{short(s.addr)}</div><div style={{fontSize:13,color:'var(--tx2)',marginTop:2}}>{s.amount} USDC - {s.freq} - Next: {s.next} {s.time||''}</div></div><button className="ap-btn ap-btn-sec" style={{fontSize:12,padding:'6px 12px'}} onClick={()=>{setSendTo(s.addr);setSendAmt(s.amount);setSendCtry(s.country);setTab('send');setStatus({type:'info',msg:'Pre-filled from schedule.'});}}>Execute</button><button className="ap-btn ap-btn-sec" style={{fontSize:12,padding:'6px 12px'}} onClick={()=>{setEditSchedId(s.id);setEditSchedData({addr:s.addr,amount:s.amount,country:s.country,freq:s.freq,next:s.next,time:s.time||'09:00'});}}>Edit</button><button className="ap-btn ap-btn-danger" onClick={()=>setScheds(p=>p.filter(x=>x.id!==s.id))}><IC.Close/></button></div>))}</div>)}{editSchedId&&editSchedData&&<div style={{position:'fixed',inset:0,zIndex:1000,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',padding:16}}><div style={{background:'var(--card)',borderRadius:20,padding:24,width:'100%',maxWidth:420,maxHeight:'90vh',overflowY:'auto'}}><div style={{fontFamily:'var(--fd)',fontWeight:800,fontSize:18,color:'var(--tx1)',marginBottom:16}}>Edit Schedule</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}><div><div className="ap-label">Recipient</div><input className="ap-input" style={{marginBottom:0}} placeholder="0x..." value={editSchedData.addr} onChange={e=>setEditSchedData(s=>({...s,addr:e.target.value}))}/></div><div><div className="ap-label">Amount (USDC)</div><input className="ap-input" style={{marginBottom:0}} type="number" placeholder="0.00" value={editSchedData.amount} onChange={e=>setEditSchedData(s=>({...s,amount:e.target.value}))}/></div><div><div className="ap-label">Country</div><CountrySelect value={editSchedData.country} onChange={v=>setEditSchedData(s=>({...s,country:v}))}/></div><div><div className="ap-label">Frequency</div><select className="ap-select" style={{marginBottom:0}} value={editSchedData.freq} onChange={e=>setEditSchedData(s=>({...s,freq:e.target.value}))}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:20}}><div><div className="ap-label">Next Date</div><input className="ap-input" style={{marginBottom:0}} type="date" value={editSchedData.next} onChange={e=>setEditSchedData(s=>({...s,next:e.target.value}))}/></div><div><div className="ap-label">Time</div><input className="ap-input" style={{marginBottom:0}} type="time" value={editSchedData.time} onChange={e=>setEditSchedData(s=>({...s,time:e.target.value}))}/></div></div><div style={{display:'flex',gap:10}}><button className="ap-btn ap-btn-sec" style={{flex:1}} onClick={()=>{setEditSchedId(null);setEditSchedData(null);}}>Cancel</button><button className="ap-btn ap-btn-primary" style={{flex:1,marginTop:0}} onClick={()=>{setScheds(p=>p.map(x=>x.id===editSchedId?{...x,...editSchedData}:x));setEditSchedId(null);setEditSchedData(null);setStatus({type:'success',msg:'Schedule updated'});}}>Done</button></div></div></div>}</div>);
