...
function AppInner() {
  ...
  const renderContacts=()=>{const addrColor=(addr)=>{const colors=['#3B82F6','#8B5CF6','#EC4899','#F59E0B','#10B981','#EF4444','#06B6D4','#F97316'];const idx=parseInt(addr.slice(2,4),16)%colors.length;return colors[idx];};const filtered=contacts.filter(c=>!cSearch||c.name.toLowerCase().includes(cSearch.toLowerCase())||c.address.toLowerCase().includes(cSearch.toLowerCase()));const recentAddrs=[...new Set(txns.slice(0,20).map(t=>t.recipient).filter(Boolean))].slice(0,5);return(<div><div className="ap-card"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><div><div className="ap-card-title">Contacts ({contacts.length})</div><div style={{fontSize:12,color:'var(--tx3)',marginTop:2}}>Your saved wallet addresses</div></div><button className="ap-btn ap-btn-primary" style={{fontSize:13,padding:'8px 16px',whiteSpace:'nowrap',width:'auto'}} onClick={()=>setShowAdd(s=>!s)}>{showAdd?'Cancel':'+ Add'}</button></div>{showAdd&&<div style={{background:'var(--elev)',borderRadius:12,padding:14,marginBottom:12}}><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}><div><div className="ap-label">Name</div><input className="ap-input" style={{marginBottom:0}} placeholder="Ahmed" value={cName} onChange={e=>setCName(e.target.value)}/></div><div><div className="ap-label">Country</div><select className="ap-select" style={{marginBottom:0}} value={cCtry} onChange={e=>setCCtry(e.target.value)}>{[<option key="" value="">Select Country</option>,...COUNTRIES.map(co=><option key={co} value={co}>{co}</option>)]}</select></div></div><div className="ap-label">Wallet Address</div><input className="ap-input" placeholder="0x..." value={cAddr} onChange={e=>setCAddr(e.target.value)} style={{marginBottom:10}}/><button className="ap-btn ap-btn-primary" style={{width:'100%'}} onClick={()=>{if(!cName||!/^0x[0-9a-fA-F]{40}$/.test(cAddr.trim())){setStatus({type:'error',msg:'Please enter a valid wallet address'});return;}setContacts(p=>[{id:Date.now(),name:cName.trim(),address:cAddr,country:cCtry},...p]);setCName('');setCAddr('');setCCtry('');setShowAdd(false);setStatus({type:'success',msg:'Contact saved'});}}>Save Contact</button></div>}{contacts.length>0&&<input value={cSearch} onChange={e=>setCSearch(e.target.value)} placeholder="Search contacts..." style={{width:'100%',padding:'9px 12px',borderRadius:10,border:'1px solid var(--b1)',background:'var(--elev)',color:'var(--tx1)',fontSize:13,outline:'none',marginBottom:12,boxSizing:'border-box'}}/> }{filtered.length===0&&contacts.length>0?<div style={{textAlign:'center',color:'var(--tx3)',padding:'24px 0',fontSize:14}}>No contacts found</div>:filtered.map(c=>(<div key={c.id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 0',borderBottom:'1px solid var(--b0)'}}><div style={{width:44,height:44,borderRadius:14,background:'var(--acd)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:800,color:'var(--ac)',flexShrink:0}}>{c.name[0].toUpperCase()}</div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:'var(--tx1)',fontSize:14,display:'flex',alignItems:'center',gap:6}}>{c.country&&<span className="ap-cc">{CC[c.country]||'?'}</span>}{c.name}</div><div style={{fontSize:11,color:'var(--tx3)',fontFamily:'monospace',marginTop:2,overflow:'hidden',textOverflow:'ellipsis'}}>{c.address}</div></div><div style={{display:'flex',gap:6,flexShrink:0}}><button className="ap-btn ap-btn-primary" style={{fontSize:12,padding:'6px 14px'}} onClick={()=>{setSendTo(c.address);setSendCtry(c.country);setTab('send');}}>Send</button><button onClick={()=>setContacts(p=>p.filter(x=>x.id!==c.id))} style={{background:'none',border:'1px solid var(--b1)',borderRadius:8,padding:'6px 8px',cursor:'pointer',color:'var(--tx3)',fontSize:12}}>✕</button></div></div>))}</div>{recentAddrs.length>0&&<div className="ap-card"><div className="ap-card-title">Recent Recipients</div><div style={{fontSize:12,color:'var(--tx3)',marginBottom:12}}>Quickly save from your recent transactions</div>{recentAddrs.map((addr,i)=>{const saved=contacts.find(c=>c.address.toLowerCase()===addr.toLowerCase());return(<div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:'1px solid var(--b0)'}}><div style={{width:44,height:44,borderRadius:14,background:addrColor(addr),display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,color:'#fff',fontWeight:700,fontSize:14}}>{saved?saved.name[0].toUpperCase():addr.slice(2,4).toUpperCase()}</div><div style={{flex:1,minWidth:0,overflow:'hidden'}}>{saved?<div style={{fontWeight:700,color:'var(--tx1)',fontSize:14,display:'flex',alignItems:'center',gap:6}}>{saved.country&&<span className="ap-cc">{CC[saved.country]||'?'}</span>}{saved.name}</div>:<div style={{fontSize:12,color:'var(--tx2)',fontFamily:'monospace',overflow:'hidden',textOverflow:'ellipsis'}}>{addr}</div>}<div style={{fontSize:11,color:'var(--tx3)',fontFamily:'monospace',marginTop:2,overflow:'hidden',textOverflow:'ellipsis'}}>{addr.slice(0,10)}...{addr.slice(-6)}</div></div><div style={{display:'flex',gap:6,flexShrink:0}}><button className="ap-btn ap-btn-primary" style={{fontSize:11,padding:'5px 10px'}} onClick={()=>{setSendTo(addr);setTab('send');}}>Send</button>{saved?<button className="ap-btn ap-btn-sec" style={{fontSize:11,padding:'5px 10px'}} onClick={()=>{setCAddr(saved.address);setCName(saved.name);setCCtry(saved.country||'');setShowAdd(true);}}>Edit</button>:<button className="ap-btn ap-btn-sec" style={{fontSize:11,padding:'5px 10px'}} onClick={()=>{setCAddr(addr);setShowAdd(true);}}>Save</button>}</div></div>);}}</div>}</div>);}
