...
function AppInner() {
  ...
  const handleMulti=async()=>{setShowConfirm(false);const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{const recipients=valid.map(r=>ethers.getAddress(r.addr.toLowerCase()));const amounts=valid.map(r=>ethers.parseUnits(parseFloat(r.amount).toString(),18));const countries=valid.map(r=>r.country||'');const total=amounts.reduce((a,b)=>a+b,0n);setStatus({type:'info',msg:'Preparing batch send...'});const iface=new ethers.Interface(['function batchSend(address[] recipients, uint256[] amounts, string[] countries) payable']);const data=iface.encodeFunctionData('batchSend',[recipients,amounts,countries]);const rp=new ethers.JsonRpcProvider(ARC_RPC||ARC_RPC_FALLBACK,{name:'Arc Testnet',chainId:ARC_CHAIN_ID});let gasHex='0x493E0';try{const est=await rp.send('eth_estimateGas',[{from:address,to:REMIT_ADDR,value:'0x'+total.toString(16),data}]);gasHex='0x'+(Math.ceil(parseInt(est,16)*1.3)).toString(16);console.log('Gas est:',parseInt(est,16),'using:',parseInt(gasHex,16));}catch(e){console.log('Gas est failed:',e.message);}const gasPriceHex='0x'+ethers.parseUnits('21','gwei').toString(16);setStatus({type:'info',msg:'Confirm batch send in your wallet...'});const txHash=await window.ethereum.request({method:'eth_sendTransaction',params:[{from:address,to:REMIT_ADDR,data,value:'0x'+total.toString(16),gas:gasHex,gasPrice:gasPriceHex}]});console.log('txHash:',txHash);setStatus({type:'info',msg:'Waiting for confirmation...'});const receipt=await provider.waitForTransaction(txHash,1,60000).catch(()=>({status:1,hash:txHash}));valid.forEach((r)=>{const rec={hash:txHash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:receipt?.status===1?'confirmed':'pending'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});if(receipt?.status===1)awardCashback(txHash);});setStatus({type:'success',msg:'Sent to '+valid.length+' recipients in one transaction!'});setMulti([{addr:'',amount:'',country:''}]);setTimeout(refreshBal,3000);setTimeout(refreshBal,6000);}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};
