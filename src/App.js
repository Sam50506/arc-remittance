...
function AppInner() {
  ...
  const handleMulti=async()=>{setShowConfirm(false);const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{const{remit}=getC();const recipients=valid.map(r=>ethers.getAddress(r.addr.toLowerCase()));const amounts=valid.map(r=>ethers.parseUnits(parseFloat(r.amount).toString(),18));const countries=valid.map(r=>r.country||'');const total=amounts.reduce((a,b)=>a+b,0n);setStatus({type:'info',msg:'Sending to '+valid.length+' recipients...'});const gasPrice=ethers.parseUnits('21','gwei');const gasLimit=200000+(valid.length*30000);const nonce=await provider.getTransactionCount(address,'pending');const data=remit.interface.encodeFunctionData('batchSend',[recipients,amounts,countries]);const txHash=await window.ethereum.request({method:'eth_sendTransaction',params:[{from:address,to:REMIT_ADDR,value:'0x'+total.toString(16),data,gas:'0x'+gasLimit.toString(16),gasPrice:'0x'+gasPrice.toString(16),nonce:'0x'+nonce.toString(16)}]});setStatus({type:'info',msg:'Waiting for confirmation...'});const receipt=await provider.waitForTransaction(txHash);if(receipt.status===1){valid.forEach((r)=>{const rec={hash:txHash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'confirmed'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});awardCashback(txHash);});setStatus({type:'success',msg:'Sent to '+valid.length+' recipients in one transaction!'});setMulti([{addr:'',amount:'',country:''}]);setTimeout(refreshBal,3000);setTimeout(refreshBal,6000);}else{setStatus({type:'error',msg:'Transaction failed'});}}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};
