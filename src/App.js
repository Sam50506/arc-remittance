...
function AppInner() {
  ...
  const[cashbackPending,setCashbackPending]=useState(0);const[seenCashback,setSeenCashback]=useState(()=>ls('arc_seen_cashback_'+address,0));const[cashbackHistory,setCashbackHistory]=useState([]);
  useEffect(()=>{
    if(!address)return;
    const fetchCashback=()=>{
  ...
  useEffect(()=>{if(tab==='history'&&signer){loadDeletedHashes().then(()=>loadContractHistory());setTxPage(1);setTxSearch('');setTxFilter('all');setExpandedTx(null);}if(signer&&address){(async()=>{try{let sched=null;for(const url of [ARC_RPC_FALLBACK,ARC_RPC_FALLBACK2,ARC_RPC_FALLBACK3]){try{const rp=new ethers.JsonRpcProvider(url,{name:'Arc Testnet',chainId:ARC_CHAIN_ID});await rp.getBlockNumber();sched=new ethers.Contract(SCHED_ADDR,SCHED_ABI,rp);break;}catch(_){}}if(!sched){sched=new ethers.Contract(SCHED_ADDR,SCHED_ABI,signer.provider||provider);}const scheduledTxns=txns.filter(t=>t.type==='scheduled'&&t.status==='scheduled');for(const st of scheduledTxns){const idMatch=st.id&&st.id.includes('_sched')?null:null;}const rawIds=await sched.getUserPayments(address);const ids=[...rawIds].map(x=>Number(x));const entries=await Promise.all(ids.map(async i=>[i,await sched.getPayment(i)]));const onChainMap=Object.fromEntries(entries);setTxns(prev=>{let changed=false;const updated=prev.map(t=>{if(t.type==='scheduled'&&t.status==='scheduled'){const match=Object.entries(onChainMap).find(([id,p])=>p.recipient.toLowerCase()===t.recipient.toLowerCase()&&Math.abs(parseFloat(ethers.formatUnits(p.amount,18))-parseFloat(t.amount))<0.001&&Number(p.releaseTime)===Number(t.releaseTime||t.timestamp));if(match){const[,p]=match;if(p.executed){changed=true;return{...t,status:'confirmed'};}if(p.cancelled){changed=true;return{...t,status:'cancelled'};}}}return t;});if(changed)lsSave('arc_txhistory_'+address,updated);return changed?updated:prev;});}catch(e){console.log('sync error',e);}})();}},[tab,signer,address]);useEffect(()=>{setStatus(null);},[tab]);useEffect(()=>{if(tab==='rewards'&&address){fetchMyClaims();setSeenCashback(cashbackPending);lsSave('arc_seen_cashback_'+address,cashbackPending);}},[tab,address]);useEffect(()=>{if(tab==='settings'&&address&&address.toLowerCase()==='0x9e086e6c07d5108ce40d84e9df1ce43caedd2306'){sbSelect('cashback_claims','status=eq.pending&select=id').then(rows=>setPendingClaimsCount(rows?.length||0)).catch(()=>{});}},[tab,address]);
        ...
        <div style={{minHeight:'100vh',background:'var(--bg)',overflowY:'auto'}}>
          ...
          <div style={{maxWidth:1100,margin:'0 auto',padding:'24px 40px',display:'flex',alignItems:'center',justifyContent:'space-between',animation:'slideUp .5s ease both'}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              ...
              <div>
                <span style={{fontFamily:'var(--fd)',fontWeight:800,fontSize:18,color:'var(--tx1)',display:'inline-flex'}}>
                  {['S','p','a','r','k','P','a','y'].map((l,i)=>(
                    <span key={i} style={{display:'inline-block',animation:'elecShake 2.5s ease infinite',animationDelay:`${(4-Math.abs(i-4))*0.08}s`}}>{l}</span>
                  ))}
                </span>
