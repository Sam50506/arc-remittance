...
.ap-btn-primary{width:100%;padding:14px;border-radius:14px;background:linear-gradient(135deg,#3B82C4,#2563A8);color:#fff;font-size:15px;box-shadow:0 4px 20px rgba(59,130,196,.28);letter-spacing:.01em;margin-top:4px}
...
.ap-input{background:var(--elev);border:1px solid var(--b1);border-radius:12px;padding:12px 14px;font-family:var(--fb);font-size:14px;color:var(--tx1);outline:none;transition:border .15s,box-shadow .15s;width:100%;display:block}
...
.ap-bot-item{flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;padding:5px 2px;cursor:pointer;font-size:10px;font-weight:600;color:var(--tx3);transition:color .14s;position:relative}
...
const ArcLogo = ({ size = 36, animated = false }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" fill="none">
    <defs>
      <radialGradient id="abg" cx="45%" cy="30%" r="75%">
        <stop offset="0%" stopColor="#1E3F7A"/>
        <stop offset="100%" stopColor="#060C1C"/>
      </radialGradient>
      <linearGradient id="ametal" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor="#E6EFF8"/>
        <stop offset="45%" stopColor="#A8C0D4"/>
        <stop offset="100%" stopColor="#628AAA"/>
      </linearGradient>
    </defs>
    <rect width="100" height="100" rx="22" fill="url(#abg)"/>
    <path d="M50 13 C50 13 27 28 24 52 L24 87 L37 87 L37 67 C37 52 43 43 50 43 C57 43 63 52 63 67 L63 87 L76 87 L76 52 C73 28 50 13 50 13 Z" fill="url(#ametal)"/>
    <path d="M50 43 C44 43 40 51 40 61 L40 87 L60 87 L60 61 C60 51 56 43 50 43 Z" fill="url(#abg)"/>
    {animated && (<>
      <g className="ap-ring-outer"><circle cx="50" cy="50" r="44" stroke="#3B82C4" strokeWidth="2" strokeDasharray="120 80" strokeLinecap="round" opacity=".5"/></g>
      <g className="ap-ring-inner"><circle cx="50" cy="50" r="30" stroke="#17E5B0" strokeWidth="2" strokeDasharray="80 52" strokeLinecap="round" opacity=".45"/></g>
    </>)}
  </svg>
);
...
function getProvider() {
  return new Promise((resolve) => {
    const tryResolve = () => {
      ...
      if(ethereum.providers?.length>0){const mm=ethereum.providers.find(p=>p.isMetaMask&&!p.isBraveWallet);if(mm)return mm;return ethereum.providers[0];}
      if(ethereum.isMetaMask||ethereum._metamask) return ethereum; return ethereum;
    };
...
function CountrySelect({value,onChange}){
  return(
    <div style={{position:'relative',display:'inline-block',flexShrink:0}}>
      <div className={`ap-country-pill${!value?' empty':''}`} style={{pointerEvents:'none',userSelect:'none'}}>
        {value?<><span className="ap-cc">{CC[value]}</span><span style={{maxWidth:72,overflow:'hidden',textOverflow:'ellipsis'}}>{value}</span></>:<span>Select country</span>}
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{marginLeft:2,flexShrink:0}}><polyline points="6 9 12 15 18 9"/></svg>
      </div>
      <select value={value} onChange={e=>onChange(e.target.value)} style={{position:'absolute',inset:0,opacity:0,cursor:'pointer',width:'100%',height:'100%',fontSize:16}}>
...
function WalletPicker({onPick,onClose}){
  const eth=window.ethereum; const options=[];
  if(eth){const providers=eth.providers?.length?eth.providers:[eth];providers.forEach((p,i)=>{let label='Browser Wallet',type='p_'+i;if(p.isMetaMask&&!p.isBraveWallet)label='MetaMask';else if(p.isBraveWallet)label='Brave Wallet';else if(p.isCoinbaseWallet)label='Coinbase Wallet';else if(p.isRabby)label='Rabby';else if(p.isTrust)label='Trust Wallet';options.push({type,label,p});});}
  else options.push({type:'install',label:'Install MetaMask'});
  ...
  return(
    <div style={{background:'var(--elev)',border:'1px solid var(--b2)',borderRadius:16,padding:20,display:'flex',flexDirection:'column',gap:10,boxShadow:'var(--shl)'}}>
      ...
      {options.map((o,i)=><button key={i} onClick={()=>onPick(o.type,o.p)} style={{display:'flex',alignItems:'center',gap:12,background:'var(--card)',border:'1px solid var(--b1)',borderRadius:12,padding:'13px 16px',cursor:'pointer',fontSize:14,fontWeight:600,color:'var(--tx1)',width:'100%',textAlign:'left',transition:'all .14s'}}>{o.type==='wc'?<IC.WC/>:<IC.Wallet/>} {o.label}</button>)}
...
export default function App() {
  ...
  const[txns,setTxns]=useState(()=>{const s=ls('arc_txhistory',[]);if(s.length===0)return[{hash:'0xdemo1',recipient:'0xda526902d606493403a0e7048790280cfbb8ee56',amount:50,country:'Pakistan',timestamp:Math.floor(Date.now()/1000)-86400,status:'confirmed'},{hash:'0xdemo2',recipient:'0x742d35Cc6634C0532925a3b8D4C9C6A21B2D4C1',amount:25,country:'India',timestamp:Math.floor(Date.now()/1000)-172800,status:'confirmed'},{hash:'0xdemo3',recipient:'0x8ba1f109551bD432803012645Ac136ddd64DBA72',amount:100,country:'Nigeria',timestamp:Math.floor(Date.now()/1000)-259200,status:'confirmed'}];return s;});
  ...
  const doDisconnect=useCallback(()=>{if(wcProvRef.current){wcProvRef.current.disconnect();wcProvRef.current=null;}setProvider(null);setSigner(null);setAddress('');setWalletName('');setStatus(null);setBalance('0.00');setIsResumed(false);lsSave('arc_session',null);},[]);

  useEffect(()=>{if(!window.ethereum)return;const onAcc=a=>{if(!a.length)doDisconnect();else setAddress(a[0]);};const onChain=()=>window.location.reload();window.ethereum.on('accountsChanged',onAcc);window.ethereum.on('chainChanged',onChain);return()=>{window.ethereum.removeListener('accountsChanged',onAcc);window.ethereum.removeListener('chainChanged',onChain);};},[doDisconnect]);
  ...
  const handleSendReview=()=>{if(!signer||!sendTo||!sendAmt){setStatus({type:'error',msg:'Please fill all required fields'});return;}if(!ethers.isAddress(sendTo)){setStatus({type:'error',msg:'Invalid recipient address'});return;}const amt=parseFloat(sendAmt);if(isNaN(amt)||amt<=0){setStatus({type:'error',msg:'Invalid amount'});return;}const rows=[{k:'You Send',v:sendAmt+' USDC'},{k:'Recipient',v:short(sendTo)}];if(sendCtry&&rates[CURRENCY[sendCtry]])rows.push({k:'They Receive',v:'~'+(amt*rates[CURRENCY[sendCtry]]).toLocaleString('en',{maximumFractionDigits:0})+' '+CURRENCY[sendCtry],highlight:true});rows.push({k:'Estimated Fee',v:'~0.001 USDC',highlight:true},{k:'Network',v:'Arc Testnet'});setConfirmData({rows,confirmLabel:'Confirm Send'});setConfirmAction(()=>handleSend);setShowConfirm(true);};

  const handleSend=async()=>{setShowConfirm(false);if(!signer||!sendTo||!sendAmt)return;const amt=parseFloat(sendAmt);if(isNaN(amt)||amt<=0)return;setLoading(true);setStatus({type:'info',msg:'Sending USDC...'});try{const value=ethers.parseUnits(sendAmt,18);const feeData=await provider.getFeeData();const gasPrice=feeData.gasPrice??ethers.parseUnits('50','gwei');const tx=await signer.sendTransaction({to:ethers.getAddress(sendTo.trim()),value,gasLimit:21000,gasPrice});const rec={hash:tx.hash,recipient:sendTo,amount:amt,country:sendCtry,timestamp:Math.floor(Date.now()/1000),status:'pending'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory',u);return u;});setStatus({type:'success',msg:'Sent '+sendAmt+' USDC to '+short(sendTo)});setSendTo('');setSendAmt('');awaitReceipt(provider,tx.hash).then(receipt=>{const confirmed=receipt==null?'pending':receipt.status===1?'confirmed':'failed';if(confirmed==='confirmed')awardCashback(tx.hash);setTxns(prev=>{const u=prev.map(t=>t.hash===tx.hash?{...t,status:confirmed}:t);lsSave('arc_txhistory',u);return u;});});setTimeout(refreshBal,6000);}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Transaction failed'});}finally{setLoading(false);}};

  const handleMultiReview=()=>{const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length){setStatus({type:'error',msg:'Add at least one valid recipient'});return;}const total=valid.reduce((s,r)=>s+(parseFloat(r.amount)||0),0);setConfirmData({rows:[{k:'Recipients',v:valid.length+' addresses'},{k:'Total',v:total.toFixed(2)+' USDC'},{k:'Est. Fee',v:'~'+(valid.length*.001).toFixed(3)+' USDC',highlight:true},{k:'Network',v:'Arc Testnet'}],confirmLabel:'Send All'});setConfirmAction(()=>handleMulti);setShowConfirm(true);};

  const handleMulti=async()=>{setShowConfirm(false);const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{for(const r of valid){setStatus({type:'info',msg:'Sending to '+short(r.addr)+'...'});const value=ethers.parseUnits(r.amount,18);const feeData=await provider.getFeeData();const gasPrice=feeData.gasPrice??ethers.parseUnits('50','gwei');const tx=await signer.sendTransaction({to:r.addr,value,gasLimit:21000,gasPrice});const rec={hash:tx.hash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'pending'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory',u);return u;});awaitReceipt(provider,tx.hash).then(receipt=>{const confirmed=receipt==null?'pending':receipt.status===1?'confirmed':'failed';if(confirmed==='confirmed')awardCashback(tx.hash);setTxns(prev=>{const u=prev.map(t=>t.hash===tx.hash?{...t,status:confirmed}:t);lsSave('arc_txhistory',u);return u;});});}setStatus({type:'success',msg:'Sent to '+valid.length+' recipients'});setMulti([{addr:'',amount:'',country:'Pakistan'}]);setTimeout(refreshBal,6000);}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};

  const handleCreateInv=async()=>{if(!address||!invPayer||!invAmt||!invDesc){setStatus({type:'error',msg:'Fill all fields and connect wallet'});return;}setLoading(true);setStatus({type:'info',msg:'Creating invoice...'});try{const id='0x'+Array.from(crypto.getRandomValues(new Uint8Array(32))).map(b=>b.toString(16).padStart(2,'0')).join('');const invoice={id,creator:address,payer:invPayer.trim(),amount:invAmt,description:invDesc,country:invCtry,paid:false};await sbInsert('invoices',invoice);const saved=ls('arc_invoices',[]);lsSave('arc_invoices',[{...invoice,desc:invDesc,ts:Math.floor(Date.now()/1000)},...saved.slice(0,99)]);setInvId(id);setStatus({type:'success',msg:'Invoice created. Share the ID with your client.'});setInvPayer('');setInvAmt('');setInvDesc('');}catch(e){setStatus({type:'error',msg:'Failed: '+e.message});}finally{setLoading(false);}};

  const handlePayInvReview=async()=>{if(!signer||!payId){setStatus({type:'error',msg:'Enter invoice ID'});return;}setLoading(true);setStatus({type:'info',msg:'Looking up invoice...'});try{const rows=await sbSelect('invoices','id=eq.'+payId.trim()+'&select=*');if(!rows||rows.length===0){setStatus({type:'error',msg:'Invoice not found'});setLoading(false);return;}const inv=rows[0];if(inv.paid){setStatus({type:'error',msg:'This invoice has already been paid'});setLoading(false);return;}setPayDet({creator:inv.creator,amount:ethers.parseUnits(inv.amount,18),description:inv.description,country:inv.country,rawAmount:inv.amount});setStatus(null);setConfirmData({rows:[{k:'Invoice ID',v:payId.trim().slice(0,16)+'...'},{k:'Amount',v:inv.amount+' USDC'},{k:'Description',v:inv.description},{k:'To',v:short(inv.creator)},{k:'Est. Fee',v:'~0.001 USDC',highlight:true}],confirmLabel:'Confirm Payment'});setConfirmAction(()=>()=>handlePayInv(inv));setShowConfirm(true);}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Lookup failed'});}finally{setLoading(false);}};

  const handlePayInv=async(inv)=>{setShowConfirm(false);if(!signer)return;setLoading(true);setStatus({type:'info',msg:'Processing payment...'});try{const value=ethers.parseUnits(inv.amount,18);const feeData=await provider.getFeeData();const gasPrice=feeData.gasPrice??ethers.parseUnits('50','gwei');const tx=await signer.sendTransaction({to:inv.creator,value,gasLimit:21000,gasPrice});await sbUpdate('invoices','id=eq.'+payId.trim(),{paid:true,paid_by:address,paid_tx:tx.hash});const rec={hash:tx.hash,recipient:inv.creator,amount:inv.amount,country:inv.country,timestamp:Math.floor(Date.now()/1000),status:'submitted'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory',u);return u;});awaitReceipt(provider,tx.hash).then(receipt=>{const confirmed=receipt==null?'pending':receipt.status===1?'confirmed':'failed';if(confirmed==='confirmed')awardCashback(tx.hash);setTxns(prev=>{const u=prev.map(t=>t.hash===tx.hash?{...t,status:confirmed}:t);lsSave('arc_txhistory',u);return u;});});setStatus({type:'success',msg:'Paid '+inv.amount+' USDC'});setPayId('');setPayDet(null);setTimeout(refreshBal,5000);}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Payment failed'});}finally{setLoading(false);}};
  ...
  const SIDEBAR_SECTIONS=[
    {title:'Transfers',items:[{id:'send',label:'Send',ICN:IC.Send,info:'Transfer USDC to any wallet instantly on Arc Testnet'},{id:'multi',label:'Multi Send',ICN:IC.Multi,info:'Send USDC to multiple recipients in one session'},{id:'invoice',label:'Invoice',ICN:IC.Invoice,info:'Create USDC payment requests stored on Supabase'},{id:'pay',label:'Pay Invoice',ICN:IC.Pay,info:'Look up and pay an invoice using its unique ID'},{id:'contacts',label:'Contacts',ICN:IC.Contacts,info:'Save wallet addresses for quick access when sending'}]},
    {title:'Analytics',items:[{id:'history',label:'History',ICN:IC.History,info:'View all your transactions with real-time status badges',dot:hasPendingTx},{id:'rates',label:'Exchange Rates',ICN:IC.Rates,info:'Live USDC to local currency conversion rates for 20 countries'},{id:'fees',label:'Fee Compare',ICN:IC.Compare,info:'See how ArcPay compares to banks and other transfer services'}]},
  ...
  const renderSend=()=>(<>
    {isResumed&&!signer&&<div className="ap-resumed-banner"><IC.Alert/>Session restored in read-only mode. Reconnect your wallet to send transactions.</div>}
    ...
    <div className="ap-send-card">
      ...
      <div className="ap-send-panel recv">
        ...
        {convertedVal?(<><div className="ap-conv-amount">{convertedVal} {CURRENCY[sendCtry]}</div><div className="ap-conv-rate">1 USDC = {rates[CURRENCY[sendCtry]]?.toFixed(2)} {CURRENCY[sendCtry]} via exchangerate-api</div></>):(<div style={{fontSize:13,color:'var(--tx3)'}}>{sendCtry?'Enter an amount above to see the conversion':'Select a destination country to see conversion estimate'}</div>)}
      </div>
    </div>
  ...
  const renderSchedule=()=>(<div><div className="ap-card"><div className="ap-card-title">Schedule Payment</div><div className="ap-card-sub">Set up recurring payment reminders.</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}><div><div className="ap-label">Recipient</div><input className="ap-input" style={{marginBottom:0}} placeholder="0x..." value={newSched.addr} onChange={e=>setNewSched(s=>({...s,addr:e.target.value}))}/></div><div><div className="ap-label">Amount (USDC)</div><input className="ap-input" style={{marginBottom:0}} type="number" placeholder="0.00" value={newSched.amount} onChange={e=>setNewSched(s=>({...s,amount:e.target.value}))}/></div><div><div className="ap-label">Country</div><select className="ap-select" style={{marginBottom:0}} value={newSched.country} onChange={e=>setNewSched(s=>({...s,country:e.target.value}))}><option value="">None</option>{COUNTRIES.map(c=><option key={c} value={c}>{c}</option>)}</select></div><div><div className="ap-label">Frequency</div><select className="ap-select" style={{marginBottom:0}} value={newSched.freq} onChange={e=>setNewSched(s=>({...s,freq:e.target.value}))}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></div></div><div className="ap-label">Next Date</div><input className="ap-input" type="date" value={newSched.next} onChange={e=>setNewSched(s=>({...s,next:e.target.value}))} style={{marginBottom:14}}/><button className="ap-btn ap-btn-primary" onClick={()=>{if(!newSched.addr||!newSched.amount||!newSched.next){setStatus({type:'error',msg:'Fill all required fields'});return;}setScheds(p=>[{id:Date.now(),...newSched},...p]);setNewSched({addr:'',amount:'',country:'Pakistan',freq:'weekly',next:''});setStatus({type:'success',msg:'Payment scheduled'});}}>Schedule Payment</button></div>{scheds.length>0&&(<div className="ap-card"><div className="ap-card-title">Active Schedules</div><div className="ap-div"/>{scheds.map(s=>(<div key={s.id} style={{display:'flex',alignItems:'center',gap:12,padding:'13px 0',borderBottom:'1px solid var(--b0)'}}><div style={{flex:1}}><div style={{fontWeight:700,color:'var(--tx1)',fontSize:14}}>{s.country&&<span className="ap-cc" style={{marginRight:6}}>{CC[s.country]||'?'}</span>}{short(s.addr)}</div><div style={{fontSize:13,color:'var(--tx2)',marginTop:2}}>{s.amount} USDC - {s.freq} - Next: {s.next}</div></div><button className="ap-btn ap-btn-sec" style={{fontSize:12,padding:'6px 12px'}} onClick={()=>{setSendTo(s.addr);setSendAmt(s.amount);setSendCtry(s.country);setTab('send');setStatus({type:'info',msg:'Pre-filled from schedule.'});}}>Execute</button><button className="ap-btn ap-btn-danger" onClick={()=>setScheds(p=>p.filter(x=>x.id!==s.id))}><IC.Close/></button></div>))}</div>)}</div>);
  ...
  const renderRates=()=>(<div className="ap-card"><div className="ap-card-title">Live Exchange Rates</div><div className="ap-card-sub">1 USDC = 1 USD via exchangerate-api.com</div><div className="ap-rates-grid">{COUNTRIES.map(c=>{const cur=CURRENCY[c],rate=rates[cur];return(<div key={c} className="ap-rate-card"><div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}><span className="ap-cc">{CC[c]}</span><span style={{fontSize:11,fontWeight:700,color:'var(--tx3)'}}>{cur}</span></div><div style={{fontSize:18,fontWeight:800,fontFamily:'var(--fd)',color:'var(--tx1)'}}>{rate?rate.toLocaleString('en',{maximumFractionDigits:1}):'...'}</div><div style={{fontSize:11,color:'var(--tx3)',marginTop:3}}>{c}</div></div>);})}</div></div>);
        ...
        <div className="ap-app">
          ...
          <main className="ap-content">
            <header className="ap-topbar">
              <div style={{display:'flex',alignItems:'center',gap:12}}><button className="ap-btn-icon mob-show" onClick={()=>setMobOpen(true)} style={{border:'none',background:'var(--elev)'}}><IC.Menu/></button><span style={{fontFamily:'var(--fd)',fontWeight:700,fontSize:16,letterSpacing:'-.2px'}}>{PAGE_TITLES[tab]||'ArcPay'}</span></div>
