...
function AppInner() {
  ...
  const handleMulti=async()=>{setShowConfirm(false);const valid=multi.filter(r=>ethers.isAddress(r.addr)&&parseFloat(r.amount)>0);if(!signer||!valid.length)return;setLoading(true);try{const{remit}=getC();const recipients=valid.map(r=>r.addr);const amounts=valid.map(r=>ethers.parseUnits(parseFloat(r.amount).toString(),18));const countries=valid.map(r=>r.country||'');const total=amounts.reduce((a,b)=>a+b,0n);setStatus({type:'info',msg:'Sending to '+valid.length+' recipients...'});let gasLimit=500000n;try{const rp=new ethers.JsonRpcProvider(ARC_RPC||ARC_RPC_FALLBACK,{name:'Arc Testnet',chainId:ARC_CHAIN_ID});const remitRO=new ethers.Contract(REMIT_ADDR,REMIT_ABI,rp);const est=await remitRO.batchSend.estimateGas(recipients,amounts,countries,{value:total,from:address});gasLimit=est*130n/100n;console.log('Gas estimated:',gasLimit.toString());}catch(e){console.log('Gas est failed, using fallback:',e.message);}const feeData=await provider.getFeeData().catch(()=>null);const gasPrice=feeData?.gasPrice||ethers.parseUnits('21','gwei');const tx=await remit.batchSend(recipients,amounts,countries,{value:total,gasLimit,gasPrice});await tx.wait();valid.forEach((r)=>{const rec={hash:tx.hash,recipient:r.addr,amount:parseFloat(r.amount),country:r.country,timestamp:Math.floor(Date.now()/1000),status:'confirmed'};setTxns(prev=>{const u=[rec,...prev.slice(0,499)];lsSave('arc_txhistory_'+address,u);return u;});awardCashback(tx.hash);});setStatus({type:'success',msg:'Sent to '+valid.length+' recipients in one transaction!'});setMulti([{addr:'',amount:'',country:''}]);setTimeout(refreshBal,3000);setTimeout(refreshBal,6000);}catch(e){setStatus({type:'error',msg:e.reason||e.message||'Failed'});}finally{setLoading(false);}};
